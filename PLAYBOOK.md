...
| 飞书/腾讯文档/Notion 等需登录的文档链接 | 登录墙文档（抓不到，走降级） |
| 其他 http(s) 链接 | 网页/博客/文章 |
| 本地文件路径（.m4a/.mp3/.wav/.aiff 等音频） | 本地录音（会议录音/语音备忘录） |
| 非链接非路径 | 纯文本（含粘贴的会议纪要/语音转文字稿），直接作为材料 |
...
### 本地录音（会议录音 / 语音备忘录 / 采访音频）

用户给出本地音频文件路径时，直接走 ASR（不需要下载步骤）：

```bash
python3 scripts/transcribe.py "<音频文件路径>" --max-seconds 900
```

- m4a/mp3/wav/aiff 均可（PyAV 直接解码，免 ffmpeg）；实测本地文件转写与
  下载音频同质量
- 会议/访谈默认用「会议纪要」或「访谈拆解」模板；说话人未分离时按内容
  推断归属并在开头声明「说话人归属由内容推断，可能有误差」
- 超过 15 分钟的录音先告知用户只转前 15 分钟（等待约 5 分钟），需要全量
  就分段跑 transcribe.py（每段调 --max-seconds 与偏移由用户确认后再做）
- faster-whisper 未安装 → 如实说明装法（pip install faster-whisper），
  不要试图用其他方式"凑合听"

### 登录墙文档（飞书 / 腾讯文档 / Notion 私有页等）

抓不到就是抓不到：这些链接在登录墙后面，fetch-article 会失败（Jina 拿到的
是登录页）。如实告诉用户两条路：
1. **导出/复制正文粘贴进来**（飞书妙记有「导出文字记录」，最快）
2. 本机若配有对应平台的 MCP/工具（如飞书 MCP）→ 借道读取，读到的文本
   照常进解读管道
不要反复换 UA/代理硬试——登录墙不是反爬，绕不过且不该绕。
